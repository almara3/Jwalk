...
def calculate_specific_SASD(single_crosslink, aa1_voxels, aa2_voxels, dens_map, aa1_CA, aa2_CA,
    ...
    visited = {}  # list works as all the coordinates that have been visited - dictionary gives the
    # path to said coordinate from startpoint
        ...
        if distance[x_n, y_n, z_n] <= max_dist:
            for c in comb:
                ...
                if (x_temp, y_temp, z_temp) not in visited:
                    if (
                        (0 <= x_temp < dens_map.x_size()) and (0 <= y_temp < dens_map.y_size()) and
                        ...
                        # if the voxel is in empty space
                        if dens_map.fullMap[z_temp][y_temp][x_temp] <= 0:
        ...
        if distance[x_n, y_n, z_n] <= max_dist:
            for c in comb:  # expand in all directions from voxel - in order of comb.
                ...
                if (x_temp, y_temp, z_temp) not in visited:
                    ...
                    if (
                        (0 <= x_temp < dens_map.x_size()) and
                        (0 <= y_temp < dens_map.y_size()) and
                        (0 <= z_temp < dens_map.z_size())
                    ):
                        ...
                        # if the voxel is in empty space
                        if dens_map.fullMap[z_temp][y_temp][x_temp] <= 0:
                            # add to queue for later searching
                            queue.append(([x_temp, y_temp, z_temp]))
    ...
    for end_residue in aa2_voxels:
        if start_residue != end_residue:
            ...
            for j in aa2_voxels[end_residue]:
                ...
                if (x, y, z) in visited:
                    # add aa1 CA voxel to path
                    visited[(x, y, z)].insert(0, aa1_CA[start_residue])
                    # add aa2 CA voxel to end of path
                    visited[(x, y, z)].append(aa2_CA[end_residue])

                    # add the distance between starting/ending residue CA voxel and start/end voxel
                    # in path
                    for i in [1, len(visited[(x, y, z)]) - 1]:
                        (x_1, y_1, z_1) = visited[(x, y, z)][i - 1]
                        (x_2, y_2, z_2) = visited[(x, y, z)][i]
            ...
            if shortest_distance != 9999:
                # this to order the dict so that chain goes alphabetically
                if start_residue[1] < end_residue[1]:
                    sasds[start_residue, end_residue, shortest_distance] = \
                        all_distances[shortest_distance]
                elif end_residue[1] < start_residue[1]:
                    sasds[end_residue, start_residue, shortest_distance] = \
                        all_distances[shortest_distance]
...
def get_euclidean_distances(sasds, pdb, aa1, aa2):
    ...
    with open(pdb) as inf:
        for line in inf:
            if line.startswith('ATOM') and (line[12:16].strip() == 'CA'):
                ...
                residues[line[22:26].strip(), chain] = [
    ...
    for s in sasds:
        if (s[0][0], s[0][1], s[1][0], s[1][1]) in euc_dists:
            sasds_and_eucs[s[0], s[1], s[2], euc_dists[(s[0][0], s[0][1], s[1][0], s[1][1])]] = \
                sasds[s]
