        ...
        if (atom.res_no, atom.chain, atom.res) in aa1_CA:
            ...
            for p in points:
                ...
                for atom2 in prot.atomList:
                    if atom2.res != atom.res or (atom2.res == atom.res and
                                                 atom2.atom_name != atom.atom_name):
                        ...
                        if tx**2 + ty**2 + tz**2 <= r**2:
                            points[p] = 1
                            break
                # if any point on the sphere doesn't intersect with another then the atom is
                # solvent accessible
                if points[p] == 0:
                    atom.clash = 0
                    break
            ...
            if atom.clash == 0:
                SA_res[atom.res_no, atom.chain, atom.res] = \
                    aa1_CA[atom.res_no, atom.chain, atom.res]

    # inform user on buried resiudes
    if xl_list:
        pass
    elif sd_res == "LYS":
        print("%d %s and 1 N-terminus of which %d are on the surface" %
              (len(aa1_CA)-1, string_dict[sd_res], len(SA_res)))
    else:
        print("%d %s of which %d are on the surface" %
              (len(aa1_CA), string_dict[sd_res], len(SA_res)))
...
def update_crosslink_pairs(crosslink_pairs, aa1_CA, aa2_CA, remove_aa1, remove_aa2):
    ...
    # for each residue pair, check both are solvent accessible
    for i in range(len(crosslink_pairs)):
        ...
        if [x1[0], x1[1]] in remove_aa1:
            ...
            if x1 not in buried_residues:
                buried_residues.append(x1)
            if x2 in remove_aa2 and x2 not in buried_residues:
                buried_residues.append(x2)
...
def check_solvent_accessibility_freesasa(prot, aa1_CA, freesasa_source="freesasa", xl_list=False):
    ...
    try:
        with open(prot[:-4]+".rsa"):
            pass
    except Exception:
        print("No .rsa file created. Please check you have Freesasa installed. Go to "
    ...
    if xl_list:
        ...
        print("%d %s and 1 N-terminus of which %d are on the surface" %
              (len(aa1_CA)-1, string_dict[sd_res], len(SA_res_dict)))
    else:
        print("%d %s of which %d are on the surface" %
              (len(aa1_CA), string_dict[sd_res], len(SA_res_dict)))
