...
def mark_CAlphas_pairs(densMap, prot, uv_xl):
    ...
    for x in aa1:
        if x not in atom_check:
            print(f"ERROR ! Residue {x[0]}-{x[1]} not in pdb structure - please check input files")
...
def generate_solvent_accessible_surface(densMap, prot, aa1_CA, aa2_CA):
    ...
    for atom in prot.atomList:
        ...
        if pos:
            ...
            if (
                ((atom.res_no, atom.chain, atom.res) in aa1_CA and
                 atom.atom_name not in backbone) or
                ((atom.res_no, atom.chain, atom.res) in aa2_CA and
                 atom.atom_name not in backbone)
            ):
                pass
            # for each atom, expand the corresponding voxel sphere around it to create solvent
            # accessible surface
            else:
                if atom.atom_name[:1] == 'C':
                    for (x, y, z) in sphere['CA']:
                        if (
                            (densMap.x_size() > (pos[0]+x) >= 0) and
                            (densMap.y_size() > (pos[1]+y) >= 0) and
...
def find_empty_space(res, sphere, densMap, CA):
    ...
    for (x_s, y_s, z_s) in sphere:
        if (
            (densMap.x_size() > (x+x_s) >= 0) and
            (densMap.y_size() > (y+y_s) >= 0) and
...
def find_surface_voxels(aa1_CA, densMap, surface, xl_list=[]):
        ...
        for y in range(-radius4, radius4+1):
            for z in range(-radius4, radius4+1):
                if (x**2 + y**2 + z**2) <= (radius4**2):
                    if (
                        ([x, y, z]) not in sphere1 and ([x, y, z]) not in sphere2 and
        ...
        for y in range(-radius6, radius6+1):
            for z in range(-radius6, radius6+1):
                if (x**2 + y**2 + z**2) <= (radius6**2):
                    if (
                        ([x, y, z]) not in sphere1 and ([x, y, z]) not in sphere2 and
                        ([x, y, z]) not in sphere3 and ([x, y, z]) not in sphere4 and
                        ([x, y, z]) not in sphere5
                    ):
    ...
    if len(buried) > 0 and len(xl_list) > 0:
        print(f"ERROR - {k_buried} buried residue(s) in xl_list:")
        for t in buried:
